...
我进行调试发现程序运行的过程中确实内存占用很多，最高可达将近5个G，确实可能会在配置低的电脑让OOM。后来又使用`pymupdf`开发了相应的功能。
不得不说`pymupdf`性能确实强很多，同样的测试文件整个过程中内存占用不到1个G，运行速度也有显著提高，并且合并后并添加水印的输出文件只用200多M（之前700M）。
